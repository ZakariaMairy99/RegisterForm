...
const Sidebar: React.FC<SidebarProps> = ({ currentStep, logoUrl = null, logoName = null }) => {
  return (
    <div className="w-full md:w-64 bg-app-surface border-b md:border-b-0 md:border-r border-app-border flex md:flex-col p-6 md:p-8 flex-shrink-0 sticky top-0 z-20 shadow-[4px_0_24px_rgba(0,0,0,0.02)]">
      {/* Logo Section */}
        ...
        {STEPS.map((step, index) => {
          ...
          return (
            <div key={index} className="group relative flex md:flex-row flex-col items-center md:items-stretch min-w-[80px] md:min-w-0 md:mb-0">
              ...
              {!isLast && (
                <div className="hidden md:block absolute left-[20px] top-[40px] bottom-[-12px] w-[2px] z-0">
              ...
              <div className="flex flex-col items-center md:flex-row md:items-center w-full z-10 md:pb-12">
                ...
                <div 
                  className={`
                    w-10 h-10 rounded-full flex items-center justify-center text-sm font-bold border-2 flex-shrink-0 transition-all duration-300 md:mr-4 mb-2 md:mb-0 shadow-sm
                    ${isCompleted 
                      ? 'bg-primary border-primary text-white shadow-primary/20' 
                      : isActive 
                        ? 'bg-app-surface border-primary text-primary shadow-md scale-105' 
                ...
                <div className={`flex flex-col items-center md:items-start transition-opacity duration-300 ${isActive ? 'opacity-100' : isCompleted ? 'opacity-80 md:opacity-100' : 'opacity-50 md:opacity-100'}`}>
                  ...
                  <span className={`
                    hidden md:block text-base font-medium leading-tight max-w-[160px]
